...
namespace RappiFixer.Helpers
{
    public class ProfitHelper
    {
        ...
        public static void PrintOutProfits(List<RappiDataModel> allRecords, List<ProductCost> productCosts)
        {
            var products = allRecords
                ...
                .Select(x => new InventoryRow()
                {
                    ...
                    Tipo = (productCosts.FirstOrDefault(y => y.PROMOCION.Trim().Equals(x.First().ProductName, StringComparison.InvariantCultureIgnoreCase))?.TIPO ?? string.Empty),
                    Cost = x.Sum(x => x.Cost),
